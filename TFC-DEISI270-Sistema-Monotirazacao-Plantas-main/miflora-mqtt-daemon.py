#!/usr/bin/env python3
import requests
from modelo import calculoClassifacao
import json
import ssl
import pandas as pd
import datetime
...
while True:
    ...
    channel_id = "2117753"
    write_api_key = "VC9648UDWAPHMR28"
    params = {
            "api_key": write_api_key,
            "field1": data["light"],
            "field2": data["temperature"],
            "field3": data["moisture"],
            "field4": data["conductivity"],
            "field5": data["battery"],
            "field7": calculoClassifacao(data["light"],data["temperature"],data["moisture"],data["conductivity"])
            
        }
    # Send the POST request to the API endpoint
    response = requests.post(url.format(channel_id), params=params)
    ...
    """
    url = 'http://192.168.1.77:8000/seu_endpoint/'
    data[classificao] = calculoClassifacao(data["light"],data["temperature"],data["moisture"],data["conductivity"])
    json_data = json.dumps(data)
    headers = {'Content-Type': 'application/json'}
    response = requests.post(url, data=json_data, headers=headers)
    # Verifica o código de status da resposta
    if response.status_code == 200:
        print('Os dados da planta foram enviados para o TFC Front End - Pedro Rocha')
    else:
        print('Erro ao aceder servidor do Pedro Rocha')
    """
